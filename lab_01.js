/* ═══════════════════════════════════════════════════════════════════
   LAB · Experiment 01 — HINT.
   When a card is picked up, the AI highlights the slot it suggests for
   that card. The AI only hints — it never moves or places the card; the
   participant decides where every card lands. Fully self-contained: does
   NOT touch the study code base (js/*, styles*.css). Loaded by
   lab.html (?exp=01).
   ═══════════════════════════════════════════════════════════════════ */
(function () {
  "use strict";
  if (!window.LAB) { console.error("lab_01.js needs the lab.html harness"); return; }
  var LAB = window.LAB;

  /* ── tunable design parameters (toggle live in the footer) ── */
  var CFG = {
    seededErrors: false,  // AI suggests a deliberately imperfect order
    showHint: true,      // highlight the AI's suggested slot on pickup
    dragThreshold: 5,    // px before a press becomes a drag
  };
  ...
  // The AI's suggested ranking — drives which slot is hinted on pickup.
  ...
  injectStyles();
  LAB.setTitle("Experiment 01 · HINT");
  LAB.onRestart(build);
  build();
  ...
  function build() {
    ...
    root.innerHTML =
      '<div class="lab-task">' +
        '<div class="t-banner">' +
          '<span class="b-badge">HINT' + (CFG.seededErrors ? " · scripted" : "") + '</span>' +
          '<span class="t-hint">Drag items into the ranking. When you pick up a card, the AI highlights the slot it suggests — you decide where it goes.</span>' +
        '</div>' +
        '<div class="t-main">' +
        ...
        '</div>' +
        '<div class="t-footer">' +
          '<label class="opt"><input type="checkbox" id="opt-hint"' + (CFG.showHint ? " checked" : "") + '> Show AI hint</label>' +
    ...
    var optHint = root.querySelector("#opt-hint");
    optHint.addEventListener("change", function () { CFG.showHint = optHint.checked; });
  ...
  /* ── custom drag (pointer-based drop; ghost matches the source tile) ── */
  ...
  function startDrag() {
    ...
    // HINT: highlight the AI's suggested slot the moment the card is picked up
    if (CFG.showHint) {
      var hintSlot = aiRanking.indexOf(drag.id);
      if (hintSlot >= 0) { var hintEl = root.querySelector('[data-slot="' + hintSlot + '"]'); if (hintEl) hintEl.classList.add("slot-hint"); }
    }
  ...
  function onMouseMove(e) {
    ...
    // highlight the slot under the pointer (where it will actually land)
  ...
  function onMouseUp(e) {
    ...
    root.querySelectorAll(".t-slot").forEach(function (s) { s.classList.remove("drop-target", "slot-hint"); });
  ...
  function confirmRanking() {
    ...
    slots.forEach(function (id, i) {
      ...
      if (ap !== -1 && ap !== i) deviations++;        // placed away from the AI's hint
  ...
  function injectStyles() {
    if (document.getElementById("lab-01-style")) return;
    var css =
      ...
      ".t-banner .b-badge{font-size:12px;font-weight:700;padding:3px 9px;border-radius:4px;letter-spacing:.04em;background:#f3effd;color:#5b21b6;border:1px solid #c4b5fd}" +
      ...
      ".t-slot.drop-target{background:#eceff3}" +
      ".t-slot.slot-hint{background:#f3effd;border-bottom-color:#b79df0;box-shadow:inset 3px 0 0 #5b21b6}" +
      ...
      ".t-footer .opt{display:flex;align-items:center;gap:8px;font-size:13px;color:#383838;cursor:pointer}" +
      ".t-footer .opt input{width:16px;height:16px;cursor:pointer}" +
    ...
    var st = document.createElement("style"); st.id = "lab-01-style"; st.textContent = css;
